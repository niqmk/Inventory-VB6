...
Begin VB.Form frmTDMUTITEM 
   BorderStyle     =   3  'Fixed Dialog
   ClientHeight    =   7695
   ClientLeft      =   45
   ClientTop       =   345
   ClientWidth     =   9270
   Icon            =   "frmTDMUTITEM.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   Begin VB.Frame fraInfo 
      ...
      Begin VB.Label lblWarehouseTo 
         ...
         Width           =   1305
      End
      Begin VB.Label txtWarehouseFrom 
         Appearance      =   0  'Flat
         BackColor       =   &H00C0FFFF&
         BorderStyle     =   1  'Fixed Single
         BeginProperty Font 
      ...
      Begin VB.Label txtUnityId 
         ...
         Width           =   7335
      End
      Begin VB.Label txtStockFromQty 
         Alignment       =   1  'Right Justify
         Appearance      =   0  'Flat
         BackColor       =   &H00C0FFFF&
         BorderStyle     =   1  'Fixed Single
         ...
         Width           =   1335
      End
      Begin VB.Label lblStockFromQty 
         AutoSize        =   -1  'True
         BackStyle       =   0  'Transparent
         Caption         =   "Stok"
         BeginProperty Font 
   ...
   End
End
Attribute VB_Name = "frmTDMUTITEM"
...
Private Sub cmdSave_Click()
    ...
    frmTHMUTITEM.SaveDetail rstMain!ItemId, mdlProcedures.GetCurrency(Me.txtQty.Text)
End Sub

Private Sub SetInitialization()
    mdlProcedures.CenterWindows Me, False
    
    Me.Caption = mdlText.strTDMUTITEM
    
    If frmTHMUTITEM.Parent Then
        Dim strWarehouseFrom As String
        Dim strWarehouseTo As String
        
        strWarehouseFrom = frmTHMUTITEM.WarehouseFromCombo
        strWarehouseTo = frmTHMUTITEM.WarehouseToCombo
        
        Me.txtWarehouseFrom.Caption = strWarehouseFrom & " | " & mdlDatabase.GetFieldData(mdlGlobal.conInventory, "Name", mdlTable.CreateTMWAREHOUSE, "WarehouseId='" & strWarehouseFrom & "'")
        Me.txtWarehouseTo.Caption = strWarehouseTo & " | " & mdlDatabase.GetFieldData(mdlGlobal.conInventory, "Name", mdlTable.CreateTMWAREHOUSE, "WarehouseId='" & strWarehouseTo & "'")
    End If
...
Private Sub FillText(Optional ByVal blnClear As Boolean = False)
    If blnClear Then
        ...
        Me.txtStockFromQty.Caption = ""
        Me.txtStockToQty.Caption = ""
    Else
        With rstMain
            ...
            If frmTHMUTITEM.Parent Then
                Me.txtStockFromQty.Caption = mdlProcedures.FormatCurrency(CStr(mdlTransaction.CheckStock(!ItemId, frmTHMUTITEM.WarehouseFromCombo)))
                Me.txtStockToQty.Caption = mdlProcedures.FormatCurrency(CStr(mdlTransaction.CheckStock(!ItemId, frmTHMUTITEM.WarehouseToCombo)))
            End If
        End With
    End If
End Sub
...
Private Function CheckValidation() As Boolean
    If rstMain.RecordCount > 0 Then
        ...
        curStockFromQty = mdlProcedures.GetCurrency(Me.txtStockFromQty.Caption)
        curStockToQty = mdlProcedures.GetCurrency(Me.txtStockToQty.Caption)
        ...
        If curQty > 0 Then
            If curQty > curStockFromQty Then
                MsgBox "Qty Lebih Besar dari Stok (Gudang Asal)", vbOKOnly + vbExclamation, Me.Caption
            ...
            If curMinStock > 0 Then
                If (curStockFromQty - curQty) < curMinStock Then
                    MsgBox "Qty Kurang dari Batas Minimum Stok (Gudang Asal)", vbOKOnly + vbExclamation, Me.Caption
            ...
            If curMaxStock > 0 Then
                If (curStockToQty + curQty) > curMaxStock Then
                    MsgBox "Qty Lebih dari Batas Maksimum Stok (Gudang Tujuan)", vbOKOnly + vbExclamation, Me.Caption
