...
Begin VB.Form frmTDFKTSELL 
   BorderStyle     =   3  'Fixed Dialog
   ClientHeight    =   6630
   ClientLeft      =   45
   ClientTop       =   345
   ClientWidth     =   7950
   Icon            =   "frmTDFKTSELL.frx":0000
   KeyPreview      =   -1  'True
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   6630
   ...
   End
End
Attribute VB_Name = "frmTDFKTSELL"
...
Private Sub cmdSave_Click()
    If rstMain.RecordCount > 0 Then
        If mdlProcedures.GetCurrency(rstMain!Qty) > 0 Then
            frmTHFKTSELL.SaveDetail rstMain!SJId
        Else
            Me.dgdMain.SetFocus
        End If
    End If
...
Private Sub SetInitialization()
    ...
    Me.Caption = mdlText.strTDFKTSELL
...
Private Sub SetGrid(Optional ByVal blnInitialize As Boolean = True)
    ...
    strTableFirst = mdlTable.CreateTHSJSELL
    strTableSecond = mdlTable.CreateTHSOSELL
    strTableThird = mdlTable.CreateTHPOSELL
    
    strTable = "(" & strTableFirst & " LEFT JOIN " & strTableSecond & _
        " ON " & strTableFirst & ".SOId=" & strTableSecond & ".SOId) LEFT JOIN " & strTableThird & _
    ...
    strCriteria = strCriteria & "CustomerId='" & frmTHFKTSELL.CustomerIdCombo & "'"
    strCriteria = strCriteria & " AND " & strTableFirst & ".SJId NOT IN (SELECT SJId FROM " & mdlTable.CreateTDFKTSELL & ")"
    ...
    With rstTemp
        While Not .EOF
            ...
            rstMain!Qty = mdlTHSJSELL.GetTotalQtySJSELL(!SJId)
...
Private Sub SetVirtualRecordset()
    ...
    With rstMain
        .Fields.Append "SJId", adChar, mdlDatabase.GetColumnSize(mdlGlobal.conInventory, "SJId", mdlTable.CreateTHSJSELL)
