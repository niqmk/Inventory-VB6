...
Begin VB.Form frmBRWTMPRICELIST 
   BorderStyle     =   3  'Fixed Dialog
   ClientHeight    =   6135
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   9270
   Icon            =   "frmBRWTMPRICELIST.frx":0000
   KeyPreview      =   -1  'True
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   6135
   ...
   End
End
Attribute VB_Name = "frmBRWTMPRICELIST"
...
Private Sub dgdMain_HeadClick(ByVal ColIndex As Integer)
    ...
    If rstMain.RecordCount > 0 Then
        frmTMPRICELIST.PriceListId = rstMain!PriceListId
    End If
End Sub
...
Private Sub cmdCancel_Click()
    If frmTMPRICELIST.Parent Then
        frmTMPRICELIST.PriceListId = strParent
    End If
...
Private Sub SetInitialization()
    ...
    Me.Caption = mdlText.strBRWTMPRICELIST
    ...
    If frmTMPRICELIST.Parent Then
        Me.txtItemId.Text = Trim(frmTMPRICELIST.ItemId)
        Me.txtPartNumber.Text = Trim(frmTMPRICELIST.PartNumber)
        Me.txtName.Text = Trim(frmTMPRICELIST.ItemName)
        
        Dim strYear As String
        Dim strMonth As String
        
        strYear = mdlProcedures.FormatDate(frmTMPRICELIST.PriceListDate, "yyyy")
        strMonth = mdlProcedures.FormatDate(frmTMPRICELIST.PriceListDate, "MM")
        ...
        strParent = Trim(frmTMPRICELIST.PriceListId)
    End If
...
Private Sub SetGrid()
    ...
    strTableFirst = mdlTable.CreateTMPRICELIST
    ...
    If mdlGlobal.objDatabaseInit = SQLSERVER7 Or mdlGlobal.objDatabaseInit = SQLSERVER2000 Or mdlGlobal.objDatabaseInit = SQLEXPRESS Then
        strCriteria = strCriteria & "PriceListDate>='" & mdlProcedures.FormatDate(Me.dtpStartDate.Value) & "' AND PriceListDate<='" & mdlProcedures.FormatDate(Me.dtpFinishDate.Value) & "'"
    ...
    Set rstMain = mdlDatabase.OpenRecordset(mdlGlobal.conInventory, "PriceListId, " & strTableFirst & ".ItemId, PartNumber, " & strTableSecond & ".Name, PriceListDate, PriceListValue, CurrencyId", strTable, False, strCriteria, "PriceListDate, " & strTableFirst & ".ItemId ASC")
    
    If rstMain.RecordCount > 0 Then
        If frmTMPRICELIST.Parent Then
            frmTMPRICELIST.ItemId = rstMain!ItemId
        End If
    End If
