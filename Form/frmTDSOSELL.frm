...
Begin VB.Form frmTDSOSELL 
   BorderStyle     =   3  'Fixed Dialog
   ClientHeight    =   8565
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   8910
   Icon            =   "frmTDSOSELL.frx":0000
   KeyPreview      =   -1  'True
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   8565
...
Private Sub txtItemPrice_GotFocus(Index As Integer)
    mdlProcedures.GotFocus Me.txtItemPrice(Index)
End Sub
...
Private Sub cmbPriceDate_Validate(Cancel As Boolean)
    ...
    Me.txtItemPrice(0).Text = mdlDatabase.GetFieldData(mdlGlobal.conInventory, "ItemPrice", mdlTable.CreateTMITEMPRICE, "PriceId='" & mdlProcedures.GetComboData(Me.cmbPriceDate, , , " : ") & "'")
...
Private Sub cmdSave_Click()
    ...
    If Me.optItemPrice(0).Value Then
        mPriceId = mdlProcedures.GetComboData(Me.cmbPriceDate, , , " : ")
    ElseIf Me.optItemPrice(1).Value Then
        Dim rstTemp As ADODB.Recordset
        
        Set rstTemp = mdlDatabase.OpenRecordset(mdlGlobal.conInventory, "*", mdlTable.CreateTMITEMPRICE, , "PriceId='" & rstMain!ItemId & mdlProcedures.FormatDate(Now, "ddMMyyyy") & "'")
    ...
    frmTHSOSELL.SaveDetail rstMain!ItemId, mdlProcedures.GetCurrency(Me.txtQty.Text), mPriceId
End Sub
...
Private Function CheckValidation() As Boolean
    If rstMain.RecordCount > 0 Then
        ...
        curMinStock = mdlProcedures.GetCurrency(mdlDatabase.GetFieldData(mdlGlobal.conInventory, "MinStock", mdlTable.CreateTMITEM, "ItemId='" & rstMain!ItemId & "'"))
        
        If curQty > 0 Then
            If curQty > curStockQty Then
                MsgBox "Stok Tidak Mencukupi", vbOKOnly + vbExclamation, Me.Caption
            ...
            Else
                If curMinStock > 0 Then
                    If (curStockQty - curQty) < curMinStock Then
                        MsgBox "Stok Minimum : " & CStr(curMinStock), vbOKOnly + vbExclamation, Me.Caption
        ...
        If Not (mdlProcedures.GetCurrency(Me.txtPOQty.Caption) >= curQty) Then
            MsgBox "Qty Melebihi Qty PO", vbOKOnly + vbExclamation, Me.Caption
...
Private Sub SetInitialization()
    ...
    Me.Caption = mdlText.strTDSOSELL
    
    blnParent = False
    
    FillCombo
    
    Me.txtItemPrice(0).Locked = True
...
Public Sub FillComboTMCURRENCY()
    ...
    Set rstTemp = mdlDatabase.OpenRecordset(mdlGlobal.conInventory, "CurrencyId", mdlTable.CreateTMCURRENCY, False, , "CurrencyId ASC")
...
Private Sub FillComboTMITEMPRICE(Optional ByVal strItemId As String = "")
    Me.cmbPriceDate.Clear
    
    Dim rstTemp As ADODB.Recordset
    
    Set rstTemp = mdlDatabase.OpenRecordset(mdlGlobal.conInventory, "PriceId, PriceDate", mdlTable.CreateTMITEMPRICE, False, "ItemId='" & strItemId & "'", "PriceDate DESC")
...
Private Sub SetGrid(Optional ByVal blnInitialize As Boolean = True)
    ...
    strTableFirst = mdlTable.CreateTDPOSELL
    ...
    strCriteria = strCriteria & "POId='" & frmTHSOSELL.POIdCombo & "'"
    
    Dim rstTemp As ADODB.Recordset
    
    Set rstTemp = mdlDatabase.OpenRecordset(mdlGlobal.conInventory, strTableFirst & ".ItemId, PartNumber, " & strTableSecond & ".Name, Qty, POId", strTable, False, strCriteria, strTableFirst & ".ItemId ASC")
    ...
    With rstTemp
        While Not .EOF
            ...
            rstMain!Name = !Name
            rstMain!Qty = mdlProcedures.GetCurrency(!Qty) - mdlTHSOSELL.GetQtyPOFromSOSELL(!POId, !ItemId)
...
Private Sub FillText(Optional ByVal blnClear As Boolean = False)
    If blnClear Then
        ...
        FillComboTMITEMPRICE
    Else
        With rstMain
            Me.txtItemIdText.Caption = !ItemId & " | " & !Name
            Me.txtPOQty.Caption = mdlProcedures.FormatCurrency(!Qty)
...
Private Sub CalculateTotalByPriceId(ByVal strPriceId As String, ByVal curItemPrice As Currency)
    ...
    strCurrencyToId = mdlDatabase.GetFieldData(mdlGlobal.conInventory, "CurrencyId", mdlTable.CreateTMITEMPRICE, "PriceId='" & strPriceId & "'")
    
    Dim mItemPrice As Currency
    
    mItemPrice = _
        mdlTransaction.ConvertCurrency( _
            frmTHSOSELL.CurrencyIdCombo, _
            strCurrencyToId, _
            mdlProcedures.GetCurrency(mdlDatabase.GetFieldData(mdlGlobal.conInventory, "ItemPrice", mdlTable.CreateTMITEMPRICE, "PriceId='" & strPriceId & "'")))
...
Private Sub CalculateTotalByCurrencyId(ByVal strCurrencyId As String, ByVal curItemPrice As Currency)
    ...
    mItemPrice = mdlTransaction.ConvertCurrency(frmTHSOSELL.CurrencyIdCombo, strCurrencyId, curItemPrice)
